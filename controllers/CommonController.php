...
{
    public function init()//首页初始化
    {
        $menu = Category::getMenu();//调用category中getMenu，得到商城分类信息
        $this->view->params['menu'] = $menu;//模板参数赋值
        $data = [];
        $data['products'] = [];
        $total = 0;
        if (Yii::$app->session['isLogin']) {
            $usermodel = User::find()->where('username = :name or useremail = :name', [":name" => Yii::$app->session['loginname']])->one();
